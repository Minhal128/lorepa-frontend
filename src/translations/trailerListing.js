export const trailersListingTranslations = {
  en: {
    price: "Price",
    type: "Type",
    keywordSearch: "Keyword search",
    trailersAvailable: "trailers available",
    noTrailersFound: "No trailers found matching your criteria.",
    popular: "Popular",
    perDay: "/Day",
    noImage: "No Image",
    lowToHigh: "Low to High",
    highToLow: "High to Low",
    travelTrailer: "Travel Trailer",
    toyHauler: "Toy Hauler",
    utility: "Utility",
    enclosed: "Enclosed",
    flatbed: "Flatbed",
    dump: "Dump",
    boat: "Boat",
    mostRecent: "Most Recent",
    unknownOwner: "Unknown Owner",
    failedToFetch: "Failed to fetch trailers",
    heroTitle: "Whether You Need a Trailer or Have One to Share",
    rentTrailerTitle: "Rent a Trailer",
    rentTrailerDescription: "Find the perfect trailer for your needs, wherever you are in Quebec. Browse, book, and go!",
    rentTrailerButton: "Rent a trailer",
    becomeHostTitle: "Become a host",
    becomeHostDescription: "List your trailer and start earning by helping others move, travel, and explore. It's easy and secure",
    bookNow: "Request a Booking",
    bookTrailer: "Book Trailer",
    startDate: "Start Date",
    endDate: "End Date",
    totalPrice: "Total Price",
    cancel: "Cancel",
    confirmBooking: "Confirm Booking",
    selectValidDates: "Please select valid start and end dates.",
    userNotFound: "User not found. Please log in.",
    submittingBooking: "Submitting booking...",
    bookingSubmittedSuccess: "Booking request submitted successfully!",
    submissionFailed: "Booking submission failed. Please try again.",
    dailyRate: "Daily Rate",
    rentalDays: "Rental Days",
    rentalPrice: "Rental Price",
    serviceFee: "Lorepa Service Fee (5%)",
    totalPayable: "Total Payable",
    totalCost: "Total Cost",
    trailerUnavailable: "Selected dates include unavailable days.",
  },
  es: {
    price: "Precio",
    type: "Tipo",
    keywordSearch: "Búsqueda por palabra clave",
    trailersAvailable: "remolques disponibles",
    noTrailersFound: "No se encontraron remolques que coincidan con sus criterios.",
    popular: "Popular",
    perDay: "/Día",
    noImage: "Sin imagen",
    lowToHigh: "De menor a mayor",
    highToLow: "De mayor a menor",
    travelTrailer: "Caravana",
    toyHauler: "Remolque de juguetes",
    utility: "Utilitario",
    enclosed: "Cerrado",
    flatbed: "Plataforma",
    dump: "Volquete",
    boat: "Barco",
    mostRecent: "Más reciente",
    unknownOwner: "Propietario desconocido",
    failedToFetch: "Error al obtener remolques",
    heroTitle: "¿Necesitas un remolque o tienes uno para compartir?",
    rentTrailerTitle: "Alquilar un remolque",
    rentTrailerDescription: "Encuentra el remolque perfecto para tus necesidades, estés donde estés en Quebec. ¡Busca, reserva y listo!",
    rentTrailerButton: "Alquilar un remolque",
    becomeHostTitle: "Conviértete en anfitrión",
    becomeHostDescription: "Publica tu remolque y comienza a ganar ayudando a otros a moverse, viajar y explorar. Es fácil y seguro.",
    bookNow: "Solicitar una reserva",
    bookTrailer: "Reservar Remolque",
    startDate: "Fecha de inicio",
    endDate: "Fecha de finalización",
    totalPrice: "Precio total",
    cancel: "Cancelar",
    confirmBooking: "Confirmar Reserva",
    selectValidDates: "Por favor, seleccione fechas de inicio y finalización válidas.",
    userNotFound: "Usuario no encontrado. Por favor, inicie sesión.",
    submittingBooking: "Enviando reserva...",
    bookingSubmittedSuccess: "¡Solicitud de reserva enviada con éxito!",
    submissionFailed: "Fallo al enviar la reserva. Por favor, inténtelo de nuevo.",
    dailyRate: "Tarifa diaria",
    rentalDays: "Días de alquiler",
    rentalPrice: "Precio de alquiler",
    serviceFee: "Tarifa de servicio Lorepa (5%)",
    totalPayable: "Total a pagar",
    totalCost: "Costo total",
    trailerUnavailable: "Las fechas seleccionadas incluyen días no disponibles.",
  },
  cn: {
    price: "价格",
    type: "类型",
    keywordSearch: "关键词搜索",
    trailersAvailable: "辆拖车可用",
    noTrailersFound: "未找到符合您条件的拖车。",
    popular: "热门",
    perDay: "/天",
    noImage: "无图片",
    lowToHigh: "从低到高",
    highToLow: "从高到低",
    travelTrailer: "旅行拖车",
    toyHauler: "玩具运输车",
    utility: "多功能拖车",
    enclosed: "封闭式拖车",
    flatbed: "平板拖车",
    dump: "自卸拖车",
    boat: "船用拖车",
    mostRecent: "最新",
    unknownOwner: "未知拥有者",
    failedToFetch: "获取拖车失败",
    heroTitle: "无论您需要拖车还是有拖车可分享",
    rentTrailerTitle: "租一辆拖车",
    rentTrailerDescription: "在魁北克无论您身在何处，都能找到满足您需求的完美拖车。浏览、预订，然后出发！",
    rentTrailerButton: "租一辆拖车",
    becomeHostTitle: "成为房东",
    becomeHostDescription: "列出您的拖车，通过帮助他人搬家、旅行和探索来开始赚钱。这既简单又安全。",
    bookNow: "请求预订",
    bookTrailer: "预订拖车",
    startDate: "开始日期",
    endDate: "结束日期",
    totalPrice: "总价",
    cancel: "取消",
    confirmBooking: "确认预订",
    selectValidDates: "请选择有效的开始和结束日期。",
    userNotFound: "未找到用户。请登录。",
    submittingBooking: "正在提交预订...",
    bookingSubmittedSuccess: "预订请求已成功提交！",
    submissionFailed: "预订提交失败。请重试。",
    dailyRate: "每日费率",
    rentalDays: "租赁天数",
    rentalPrice: "租赁价格",
    serviceFee: "Lorepa服务费 (5%)",
    totalPayable: "应付总额",
    totalCost: "总费用",
    trailerUnavailable: "所选日期包含不可用的日期。",
  },
  fr: {
    price: "Prix",
    type: "Type",
    keywordSearch: "Recherche par mot-clé",
    trailersAvailable: "remorques disponibles",
    noTrailersFound: "Aucune remorque trouvée correspondant à vos critères.",
    popular: "Populaire",
    perDay: "/Jour",
    noImage: "Pas d'image",
    lowToHigh: "Du plus bas au plus élevé",
    highToLow: "Du plus élevé au plus bas",
    travelTrailer: "Caravane",
    toyHauler: "Remorque pour jouets",
    utility: "Utilitaire",
    enclosed: "Fermée",
    flatbed: "Plateforme",
    dump: "Benne",
    boat: "Bateau",
    mostRecent: "Plus récent",
    unknownOwner: "Propriétaire inconnu",
    failedToFetch: "Échec du chargement des remorques",
    heroTitle: "Que vous ayez besoin d'une remorque ou que vous en ayez une à partager",
    rentTrailerTitle: "Louer une remorque",
    rentTrailerDescription: "Trouvez la remorque parfaite pour vos besoins, où que vous soyez au Québec. Parcourez, réservez et partez !",
    rentTrailerButton: "Louer une remorque",
    becomeHostTitle: "Devenir hôte",
    becomeHostDescription: "Répertoriez votre remorque et commencez à gagner de l'argent en aidant les autres à déménager, voyager et explorer. C'est facile et sécurisé.",
    bookNow: "Demander une réservation",
    bookTrailer: "Réserver la Remorque",
    startDate: "Date de début",
    endDate: "Date de fin",
    totalPrice: "Prix total",
    cancel: "Annuler",
    confirmBooking: "Confirmer la Réservation",
    selectValidDates: "Veuillez sélectionner des dates de début et de fin valides.",
    userNotFound: "Utilisateur non trouvé. Veuillez vous connecter.",
    submittingBooking: "Soumission de la réservation...",
    bookingSubmittedSuccess: "Demande de réservation soumise avec succès !",
    submissionFailed: "Échec de la soumission de la réservation. Veuillez réessayer.",
    dailyRate: "Tarif journalier",
    rentalDays: "Jours de location",
    rentalPrice: "Prix de location",
    serviceFee: "Frais de service Lorepa (5%)",
    totalPayable: "Total à payer",
    totalCost: "Coût total",
    trailerUnavailable: "Les dates sélectionnées incluent des jours indisponibles.",
  }
};