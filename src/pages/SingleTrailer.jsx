...
const SingleTrailer = () => {
  ...
  const validateKycBeforeBooking = async () => {
    ...
    try {
      ...
      if (!isKycVerified) {
        toast.error(translations2.kycVerificationRequired || "Your account is not verified by the admin yet. You can book once it is approved.");
  ...
  const handleBookingSubmit = async ({ trailerId, startDate, endDate, price, message }) => {
    ...
    try {
      ...
      if (error?.response?.status === 403) {
        toast.error(translations2.kycVerificationRequired || apiMessage || "Your account is not verified by the admin yet. You can book once it is approved.", { id: loadingToast });
        nav('/user/dashboard/profile?tab=documents');
        return;
      }
